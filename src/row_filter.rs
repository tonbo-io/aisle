...
///
/// **Feature gate**: This module is only available when the `row_filter` feature is enabled.
///
/// ```toml
/// [dependencies]
...
/// ```
///
/// This module provides two row filter implementations:
/// - [`IrRowFilter`]: Evaluates compiled IR expressions (best for known-supported predicates)
/// - [`ExprRowFilter`]: Evaluates DataFusion expressions for supported operators
///   (exact for this subset; returns errors for unsupported expressions)
///
/// Use [`ExprRowFilter`] when you need exact filtering for supported predicates.
/// Use [`IrRowFilter`] when you have pre-compiled IR and know it's complete.
...
use datafusion_expr::{Between, BinaryExpr, Expr, Operator, expr::InList, utils::expr_to_columns};
use parquet::{
    arrow::{ProjectionMask, arrow_reader::ArrowPredicate},
    schema::types::SchemaDescriptor,
};

use crate::ir::{CmpOp, IrExpr};

type Result<T> = std::result::Result<T, ArrowError>;

/// Wrapper for using IrExpr as a row filter with Parquet's built-in filtering
pub struct IrRowFilter {
    expr: IrExpr,
...
impl ArrowPredicate for IrRowFilter {
...
///
/// Unlike [`IrRowFilter`], this evaluates the expression tree at the row level for the
...
impl ExprRowFilter {
    ...
    /// * `parquet_schema` - Parquet schema descriptor (needed for projection)
    pub fn new(expr: Expr, schema: Arc<Schema>, parquet_schema: &SchemaDescriptor) -> Self {
    ...
    fn evaluate_expr(&self, expr: &Expr, batch: &RecordBatch) -> Result<BooleanArray> {
        match expr {
            // Literal boolean
            ...
            // LIKE
            Expr::Like(like) => self.evaluate_like(batch, &like.expr, &like.pattern, like.negated),

            // CAST
            Expr::Cast(cast) => self.evaluate_cast_expr(&cast.expr, &cast.data_type, false, batch),
            Expr::TryCast(cast) => {
    ...
    fn evaluate_value(&self, expr: &Expr, batch: &RecordBatch) -> Result<ArrayRef> {
        match expr {
            Expr::Column(col) => batch
                .column_by_name(&col.name)
                .cloned()
                .ok_or_else(|| {
                    ArrowError::SchemaError(format!("Column '{}' not found", col.name))
                }),
            Expr::Literal(val, _) => Ok(val.to_array_of_size(batch.num_rows())?),
            Expr::Cast(cast) => {
            ...
            Expr::Alias(alias) => self.evaluate_value(&alias.expr, batch),
            _ => Err(ArrowError::NotYetImplemented(format!(
    ...
    fn cast_array(
        ...
        let options = CastOptions { safe, ..Default::default() };
...
fn projection_from_expr(expr: &Expr, parquet_schema: &SchemaDescriptor) -> ProjectionMask {
    let mut columns = HashSet::new();
...
fn collect_expr_columns(expr: &Expr, columns: &mut HashSet<String>) -> bool {
...
mod tests {
    ...
    use datafusion_expr::{BinaryExpr, Expr, Operator, expr::{InList, Like}};
    use parquet::{
    ...
    fn test_equality_filter() {
        ...
        let mut filter = IrRowFilter::new(expr, &schema_descr(&schema));
    ...
    fn test_expr_like_large_utf8() {
        ...
        let mut filter = ExprRowFilter::new(expr, schema.clone(), &schema_descr(&schema));
        let result = filter.evaluate(batch).unwrap();
    ...
    fn test_expr_in_list_cast_utf8() {
        ...
        let expr = Expr::InList(InList {
            ...
            list: vec![
                Expr::Literal(ScalarValue::Utf8(Some("2".to_string())), None),
                Expr::Literal(ScalarValue::Utf8(Some("4".to_string())), None),
            ],
            negated: false,
        });
    ...
    fn test_expr_is_null_cast() {
        ...
        let expr = Expr::IsNull(Box::new(cast_expr));
